...
export default class App extends Component {
            ...
            fetch(`https://apiv1.zis.ru/tests/avg_data/${id}?date_begin=${date_begin}&date_end=${date_end}&period=${chart_type}&data_type=power`, {
                ...
                if (response.hasOwnProperty('statusCode') && response.statusCode == 400 || response.hasOwnProperty('statusCode') && response.statusCode == 403) {
                    ...
                    await this.setState({
                        ...
                        voltage_item_info: !response.avg ? 0 : response.avg
                    })
                }
                await callback()
            })
    ...
    setDayData = async () => {
        ...
        for (const item in chartData)
        {
            ...
            chartData[item].power =  parseFloat(chartData[item].power);
        }
        ...
        //         chartData1[day_without_zero][1] = parseFloat(chartData[item].power);
        //     } else {
        //         chartData1[day_without_zero-1][1] = parseFloat(chartData[item].power);
        //     }
        // }
        //
        //
                        ...
                        ${type}.addEventListener("message", message => {
                          ...
                          google.charts.setOnLoadCallback(drawVisualization);
                            ...
                            //Готовим массив под наполнение
                              ...
                              dataArray.push([
                                      caption,  //Заголовок
                                        element.power//Целевое значение
                                        ]);
                            });
                            ...
                            <View style={styles.impulse_surges_item_img_dates_info_wrapper}>
                                ...
                                <View style={styles.impulse_surges_items_second_wrapper}>
                                    ...
                                    <View style={styles.impulse_surges_item}>
                                        ...
                                        <Text style={styles.impulse_surges_item_info}>{this.state.minimum_info} {this.state.language.vt}</Text>
                                    </View>

                                    <View style={styles.impulse_surges_item}>
                                        <Text style={styles.impulse_surges_item_title}>{this.state.language.maximum}</Text>
                                        <Text style={styles.impulse_surges_item_info}>{this.state.maximum_info} {this.state.language.vt}</Text>
                                    </View>

                                    <View style={styles.impulse_surges_item}>
                                        <Text style={styles.impulse_surges_item_title}>{this.state.language.average}</Text>
                                        <Text style={styles.impulse_surges_item_info}>{this.state.voltage_item_info} {this.state.language.vt}</Text>
                                    </View>
