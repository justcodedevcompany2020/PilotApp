...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            limit: this.props.undervoltage_limit,
            lower_voltage_trigger: this.props.lower_voltage_trigger,
            peak_value: 0,
            todayDate: '',
            total_duration: 0,
    ...
    getChartData = async (callback) => {
        ...
        // console.log(`https://apiv1.zis.ru/tests/voltage_problems/4?date_begin=${date_begin}&date_end=${date_end}&period=${chart_type}&data_type=undervoltage`)
    ...
    setDayData = async () => {
        ...
        let {chartData, chart_type,lower_voltage_trigger, language_name } = this.state;
        ...
        let jsonChartData1 = JSON.stringify({data: chartData, chart_type: chart_type, lower_voltage_trigger: lower_voltage_trigger, language_name:language_name});
        this.webviewRef.current.postMessage(jsonChartData1);

        console.log(jsonChartData1, 'jsonChartData1jsonChartData1')

        //
        // let {chartData,timezone} = this.state;
        //
        // for (const item in chartData)
        // {
        //     let timestamp = chartData[item].timestamp;
        //     // console.log(timestamp, 'timestamptimestamp start');
        //     // console.log(timezone, 'timezone start');
        //     //
        //     // if(timezone < 0) {
        //     //     let absolute_timezone = Math.abs(timezone);
        //     //     timestamp = moment(timestamp, "YYYY-MM-DDTHH:mm:ss")
        //     //         .subtract(absolute_timezone, 'hours').utc().format();
        //     // } else {
        //     //     timestamp = moment(timestamp, "YYYY-MM-DDTHH:mm:ss").add(3, 'hours').utc().format();
        //     //     alert()
        //     // }
        //     //
        //     // // timestamp = timestamp;
        ...
        //     // console.log(timestamp, 'timestamptimestamp converted');
        ...
        //     chartData1.push( [i.toString(), this.state.limit, this.state.limit])
        // }
        //
        // for (const item in chartData)
        // {
    ...
    setWeekData = async () => {
        ...
        let jsonChartData1 = JSON.stringify({
            ...
            lower_voltage_trigger: lower_voltage_trigger,
        ...
        //     }
        //
        //     chartData1.push( [week_name, this.state.limit, this.state.limit])
        // }
        //
        // console.log(chartData1, 'chartData1');
        //
        ...
        //
        // let chartData1 = [0,0,0,0,0,0,0];
        //
        // for (const item in chartData)
        // {
        //     // chartData1.push(chartData[item].voltage);
        //     let week_day_data = chartData1[chartData[item].timestamp2];
        //     week_day_data = parseFloat(week_day_data) + parseFloat(chartData[item].voltage);
        //
        //     chartData1[chartData[item].timestamp2] = week_day_data;
        //
        //     // chartData1[chartData[item].timestamp2].push(chartData[item].voltage);
        //
        // }
        //
        // let chartLabels = [];
    ...
    setMonthData = async () => {
        ...
        let {chartData,chart_type, lower_voltage_trigger, language_name:language_name} = this.state;
                    ...
                    <View style={styles.all_devices_general_page_header}>
                        <View style={styles.all_devices_general_page_header_child}>
                            ...
                            <TouchableOpacity style={styles.title_back_btn_wrapper} onPress={() => {this.redirectToTestReport()}}>
                                ...
                                <Text style={styles.all_devices_general_page_header_title}>{this.state.language.undervoltage}</Text>
                            </TouchableOpacity>
